...
import { exists, readJsonFile, writeJsonFile, writeTextFile } from "../lib/fs.js";
...
function chooseMarketingAsset(pack: AssetPackRecord, channel: GrowthChannel): string {
  ...
  if (channel === "x" || channel === "gumroad_update") {
    return path.join(marketingRoot, "teaser-landscape.png");
  }

  return path.join(marketingRoot, "teaser-square.png");
}
...
export class StoreOpsService {
  ...
  selectNextAssetType(
    ...
    return underCap?.assetType ?? viable[0].assetType;
  }
  ...
  async buildRevenueSnapshot(
    ...
    const snapshot: RevenueAllocationSnapshot = {
      id: `${businessId}-revenue-${new Date().toISOString().replaceAll(":", "-")}`,
      businessId,

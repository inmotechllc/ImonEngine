...
export class StoreAutopilotAgent {
  ...
  private composeOperationsRunbook(): string {
    return [
      ...
      "- `scripts/publish_growth_post.py` prefers the Meta Graph API for Facebook when `META_PAGE_ACCESS_TOKEN` is present, and otherwise falls back to the signed-in Facebook Page UI using the business-scoped social profile instead of the generic Meta Business Suite composer.",
  ...
  private composeEnvExample(): string {
    return [
      ...
      "",
      "# Shared AI routing and engine",
      "AI_PROVIDER_OPENAI_API_KEY=",
      "AI_PROVIDER_OPENAI_BASE_URL=",
      "AI_PROVIDER_NVIDIA_API_KEY=",
      "AI_PROVIDER_NVIDIA_BASE_URL=",
      "AI_PROVIDER_LOCAL_API_KEY=",
      "AI_PROVIDER_LOCAL_BASE_URL=",
      "AI_PROVIDER_NOMI_API_KEY=",
      "AI_PROVIDER_NOMI_BASE_URL=http://127.0.0.1:1100",
      "# Shared and business-specific AI route assignments live in src/ai/api-map.ts.",
      "# Shared fast, deep, and research routes now use Nomi by default.",
      "# Private files under Secrets/*.env are loaded after .env; keep live gateway keys there or in host env storage.",
      "# NVIDIA chat-completions routes are still configured for legacy/provider fallback work and default to https://integrate.api.nvidia.com/v1 when no base URL override is set.",
      "# Nomi gateway routes use /requests and can also read NOMI_GATEWAY_* or OMIN_NOMI_GATEWAY_* during migration.",
      "# These legacy OpenAI keys still load during the transition window.",
      "OPENAI_API_KEY=",
      "OPENAI_MODEL_FAST=",
      "OPENAI_MODEL_DEEP=",
      ...
      "# NORTHLINE_PROSPECT_SOURCE_DIR=/opt/imon-engine/runtime/prospect-sources/northline",
